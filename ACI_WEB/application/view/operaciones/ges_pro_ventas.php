...
<table id="products" class="table table-striped" cellspacing="0"  >
     ...
     $Item =  $this->model->get_ProductsList(); 
        ...
        echo  "<tr><td width='5%'><a title='Agregar a la orden' data-toggle='modal' data-target='#myModal' href='javascript:void(0)' onclick='javascript: modal(".$ID.",".$NAME.",".$PRICE."); ' ><i style='color:green' class='fa fa-plus'></i></a></td>
            ...
            <td width="20%" class="numb" id='.$Item->{'ProductID'}.$Item->{'Description'}.'-price'.">".number_format($Item->{'Price1'}, 2, '.', ',').'</td>
...
<div class="col-lg-12">
 <div   class="col-lg-6"> 
    ...
    <label style="display:inline"> Factura No. </label><INPUT class="input-control" type="text" name="no_order" id="no_order" readonly value="
     <?php echo  $this->model->Get_Order_No(); ?>" />
</div>
...
function agregar_pro_sale_sale(id,item,price,lote,venc,ruta,max,uni){
		...
		if(qty!=''){
           ...
           var arrayItem =LineArray.length;
	        ...
	        LineArray[LineArray.length] = id+"/"+unit_price+"/"+document.getElementById('no_order').value+"/"+qty+"/"+price+"/"+lote+'/'+ruta+'/'+FechaVenc;
					...
					alert("La cantidad seleccionada no puede ser mayor a la cantidad en stock");
			...
			if(LineArray!=''){
				...
				var r = confirm("Este seguro de crear esta factura de venta?");
				...
				if(SalesOrderNumber && CustomerID ){
							...
							var datos= "url=bridge_query/set_sales_header/"+ID_compania+'/'+SalesOrderNumber+'/'+CustomerID+'/'+Subtotal+'/'+TaxID+'/'+total+'/'+user;
				...
				$.each(LineArray, function(index,value) {
                           ...
                           setTimeout( function(){ 
                               	...
                               	count++;
											...
											var datos= "url=bridge_query/set_sales_detail/"+ID_compania+'/'+value+'/'+LineArray.length+'/'+count;
				...
				alert('Debe incluir al menos un Item en la factura');
	     	...
	     	function msg(link,SalesOrderNumber){
			    ...
			    alert("La factura se ha enviado con exito");

			    var R = confirm('Desea imprimir la factura de venta?');

			    if(R==true){


                 window.open(link+'?url=ges_ventas/ges_print_sales/'+SalesOrderNumber,'_self');
